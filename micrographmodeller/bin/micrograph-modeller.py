...
if __name__ == "__main__":
    ...
    try:
        ...
        oversampling = config["General"].getint(
            "Oversampling"
        )  # oversampling is used for correcting
        ...
        if (simulator_mode in ["TiltSeries", "FrameSeries"]) or (
            device in ["CPU", "GPU"]
        ):
    ...
    if "GenerateModel" in config.sections():
        try:
            ...
            thickness = (
                draw_range(
                    literal_eval(config["GenerateModel"]["Thickness"]),
                    float,
                    "Thickness",
                )
                * 1e-9
            ...
            )  # calculate thickness in number of voxels!
            # make even number to solve tomogram reconstruction mismatch bug
            thickness_voxels -= thickness_voxels % 4
            # gold markers
            number_of_markers = draw_range(
                literal_eval(config["GenerateModel"]["NumberOfMarkers"]),
                int,
                "NumberOfMarkers",
            )
            # parse range of number of particles
            number_of_particles = draw_range(
                literal_eval(config["GenerateModel"]["NumberOfParticles"]),
                int,
                "NumberOfParticles",
            )
            number_of_membranes = draw_range(
                literal_eval(config["GenerateModel"]["NumberOfMembranes"]),
                int,
                "NumberOfMembranes",
            )
            sigma_motion_blur = config["GenerateModel"].getfloat(
                "SigmaMotionBlur"
            )  # in A units
    ...
    if "Microscope" in config.sections():
        try:
            ...
            # beam damage SNR
            beam_damage_snr = draw_range(
                literal_eval(config["Microscope"]["BeamDamageSNR"]),
                float,
                "BeamDamageSNR",
            )
            defocus = (
                draw_range(
                    literal_eval(config["Microscope"]["Defocus"]), float, "Defocus"
                )
                * 1e-6
            )
            electron_dose = draw_range(
                literal_eval(config["Microscope"]["ElectronDose"]),
                float,
                "ElectronDose",
            )
            spherical_aberration = (
                config["Microscope"].getfloat("SphericalAberration") * 1e-3
            )
            chromatic_aberration = (
                config["Microscope"].getfloat("ChromaticAberration") * 1e-3
            )
            energy_spread = config["Microscope"].getfloat("EnergySpread")
            illumination_aperture = (
                config["Microscope"].getfloat("IlluminationAperture") * 1e-3
            )
            objective_diameter = (
                config["Microscope"].getfloat("ObjectiveDiameter") * 1e-6
            )
            focus_length = config["Microscope"].getfloat("FocalDistance") * 1e-3
            astigmatism = config["Microscope"].getfloat("Astigmatism") * 1e-9
            astigmatism_angle = draw_range(
                literal_eval(config["Microscope"]["AstigmatismAngle"]),
                float,
                "AstigmatismAngle",
            )
        except Exception as e:
    ...
    if simulator_mode in config.sections():
        try:
            ...
            translation_shift = draw_range(
                literal_eval(config[simulator_mode]["TranslationalShift"]),
                float,
                "TranslationalShift",
            )
    ...
    if "TomogramReconstruction" in config.sections():
        try:
            ...
            use_scaled_projections = config["TomogramReconstruction"].getboolean(
                "UseScaledProjections"
            )
            align = config["TomogramReconstruction"].getboolean("Align")
        except Exception as e:
            print(e)
            raise Exception(
                "Missing tomogram reconstruction parameters in config file."
            )
    ...
    logging.basicConfig(
        filename="{}/simulator-{date:%Y-%m-%d_%H:%M:%S}.log".format(
            save_path, date=datetime.datetime.now()
        ),
        level=logging.INFO,
    )
    ...
    logging.info(
        "Values of parameters that we randomly vary per simulation (only for generate model and generate projections):"
    )
    if "GenerateModel" in config.sections():
        logging.info(
            f"model thickness = {thickness_voxels*pixel_size*1E9:.2f}nm (adjusted to be an even number of voxels)"
        )
