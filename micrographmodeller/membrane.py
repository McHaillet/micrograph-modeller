...
try:
    ...
    print(
        "Import of pyvista for generating triangulation is not possible without graphical backend"
    )
...
class Vector:
    ...
    def __init__(self, coordinates, normalize=False):
        ...
        assert (
            len(coordinates) == 3
        ), "Invalid axis list for a 3d vector, input does not contain 3 coordinates."
    ...
    def cross(self, other):
        ...
        return Vector(
            [
                self._axis[1] * other._axis[2] - self._axis[2] * other._axis[1],
    ...
    def dot(self, other):
        ...
        """
        return (
            self._axis[0] * other._axis[0]
            + self._axis[1] * other._axis[1]
            + self._axis[2] * other._axis[2]
        )
    ...
    def _get_orthogonal_unit_vector(self):
        ...
        if self._zero_vector:
            ...
            np.testing.assert_allclose(
                ...
                0,
                atol=1e-7,
                err_msg="something went wrong in finding " "perpendicular vector",
            )
            return orth
...
def place_back_to_ellipsoid(point, a, b, c, maxiter=20):
    ...
    assert (
        a >= b >= c > 0
    ), "finding point currently only possible on ellipsoid with a >= b >= c > 0"
    ...
    for i, x in enumerate(np.linspace(-a, a, xx)):
        for j, y in enumerate(np.linspace(-b, b, yy)):
            for k, z in enumerate(np.linspace(-c, c, zz)):
                ...
                ellipsoid_point = place_back_to_ellipsoid(
                    point, a, b, c, maxiter=iterations
                )
                distances[i, j, k] = np.sqrt(
                    distance.sqeuclidean(point, ellipsoid_point)
                )
    # visualize
...
def point_array_sign(point_array, p2, p3):
    ...
    """
    return (point_array[:, 0] - p3[0]) * (p2[1] - p3[1]) - (p2[0] - p3[0]) * (
        point_array[:, 1] - p3[1]
    )
...
class Triangle:
    def __init__(self, points, normal):
        assert isinstance(points[0], np.ndarray) and (
            len(points[0]) == 3
        ), "invalid triangle points provided"
        self.p1, self.p2, self.p3 = (
            np.append(points[0], 1.0),
            np.append(points[1], 1.0),
            np.append(points[2], 1.0),
        )
    ...
    def get_transformed_triangle(self, matrix):
        new_points = [
            np.dot(self.p1, matrix)[:3],
            np.dot(self.p2, matrix)[:3],
            np.dot(self.p3, matrix)[:3],
        ]
    ...
    def get_shifted_points(self, shift):
        return np.vstack(
            [self.p1[:3] + shift, self.p2[:3] + shift, self.p3[:3] + shift]
        )
    ...
    def get_min(self):
        return np.array(
            [min([p[i] for p in (self.p1, self.p2, self.p3)]) for i in [0, 1, 2]]
        )

    def get_max(self):
        return np.array(
            [max([p[i] for p in (self.p1, self.p2, self.p3)]) for i in [0, 1, 2]]
        )
...
class DistanceMatrix:
    ...
    def update(self, points, new_point_index):
        dist_update = np.sum(
            (points - points[new_point_index]) ** 2, axis=1
        )  # squared euclidean (see above)
        dist_update[dist_update == 0] = (
            self.upper
...
class Vesicle:
    def __init__(self, radius, voxel_spacing):
        ...
        self.reference_normal = Vector(
            [0.0, 0.0, 1.0]
        )  # dont see the point of making this an init param
        self.point_cloud = None
        self.framework = None  # list of triangles?
        self.occupation = (
            None  # bool list of length n_triangles; true if membrane protein is placed
        )
    ...
    def equilibrate_point_cloud(self, maxiter=10000, factor=0.01, display=False):
        ...
        for x in range(maxiter):
            ...
            scale = factor * (
                mean_dist / distance.euclidean(p1, p2)
            )  # make scaling larger for tiny distances
    ...
    def generate_framework(self, alpha):
        ...
        for i in range(
            shell.n_cells
        ):  # make each surface cell a triangle in the framework
            self.framework.append(
                Triangle(shell.extract_cells(i).points, shell.cell_normals[i])
            )
    ...
    def sample_membrane(self, bilayer_pdb, cores=1):
        ...
        (
            ...
            b_factors,
            occupancies,
        ) = map(np.array, read_structure(bilayer_pdb))
        ...
        if "blas" in user_apis:
            print(
                "BLAS in multithreading user apis, will try to limit number of threads for numpy.dot"
            )
        ...
        for i_cell, triangle in enumerate(self.framework):
            ...
            mat_ref_rot = self.reference_normal.get_rotation(
                triangle.normal, as_affine_matrix=True
            )
            mat_plane_rot = rotation_matrix(
                (np.random.uniform(0, 360), 0, 0), rotation_order="rzxz"
            )

            # get the transformed Triangle
            transf_triangle = triangle.get_transformed_triangle(
                np.dot(np.dot(mat_origin_shift, mat_ref_rot), mat_plane_rot)
            )

            # find the shift so that triangle is in the positive quadrant of the xy axes
            shift_xy = np.append(
                transf_triangle.get_min()[:2], 0
            )  # already in xy plane so zshift should be 0
            mat_xy_shift = translation_matrix(shift_xy).T

            # construct the full transformation matrix to sample from the lipid layer
            matrix = np.dot(
                np.dot(mat_origin_shift, mat_ref_rot),
                np.dot(mat_plane_rot, mat_xy_shift),
            )
            ...
            atoms = np.empty(
                (xext * yext * n_atoms_box, 4)
            )  # numpy atom array with added dim for matrix mult
            ...
            for i in range(xext):
                for j in range(yext):
                    ...
                    atoms[index * n_atoms_box : (index + 1) * n_atoms_box, :] = (
                        np.array(
                            [
                                x_coordinates + i * x_bound,
                    ...
                    newe[index * n_atoms_box : (index + 1) * n_atoms_box] = (
                        elements.copy()
                    )
                    newb[index * n_atoms_box : (index + 1) * n_atoms_box] = (
                        b_factors.copy()
                    )
                    newo[index * n_atoms_box : (index + 1) * n_atoms_box] = (
                        occupancies.copy()
                    )
            ...
            if "blas" in user_apis:
                with threadpool_limits(limits=cores, user_api="blas"):
                    ratoms = np.dot(
                        atoms, np.linalg.inv(matrix)
                    )  # multiply the atoms with the matrix inverse
            else:
            ...
            if i_cell % 500 == 0:
                print(
                    f"At triangle {i_cell + 1} out of {len(self.framework)}. Current atom count is "
                    f"{atom_count}."
                )
    ...
    def display_point_cloud(self, zlim=None):
        ...
        ), "point cloud not yet initialized, cannot display"
        ...
        ax = fig.add_subplot(111, projection="3d")
        ax.scatter(
            self.point_cloud[:, 0], self.point_cloud[:, 1], self.point_cloud[:, 2]
        )
        if zlim is not None:
