...
V_WATER = (
    4.5301  # potential value of low density amorphous ice (from Vulovic et al., 2013)
)
...
def potential_amplitude(rho, molecular_weight, voltage):
    ...
    sigma_transfer = (
        2 * np.pi * relative_mass * constants["el"] * Lambda / (constants["h"] ** 2)
    )
    ...
    if molecular_weight == WATER_MW:  # water molecule
        ...
        sigma_inelastic_H = (
            8.8e-6
            * beta2_100
            * np.log(beta2 * (voltage + E0) / (E_loss / 2))
            / (beta2 * np.log(beta2_100 * (100e3 + E0) / (E_loss / 2)))
        )
        sigma_inelastic_O = (
            1.5 * 1e-6 * ZO**0.5 / beta2 * np.log(beta2 * (voltage + E0) / (E_loss / 2))
        )
        sigma_inelastic = 2 * sigma_inelastic_H + sigma_inelastic_O
    elif molecular_weight == CARBON_MW:  # carbon film
        ZC = 6
        sigma_inelastic = (
            1.5 * 1e-6 * ZC**0.5 / beta2 * np.log(beta2 * (voltage + E0) / (E_loss / 2))
        )
        ...
        sigma_inelastic_N = (
            1.5 * 1e-6 * ZN**0.5 / beta2 * np.log(beta2 * (voltage + E0) / (E_loss / 2))
        )
        sigma_inelastic_O = (
            1.5 * 1e-6 * ZO**0.5 / beta2 * np.log(beta2 * (voltage + E0) / (E_loss / 2))
        )
        sigma_inelastic_P = (
            1.5 * 1e-6 * ZP**0.5 / beta2 * np.log(beta2 * (voltage + E0) / (E_loss / 2))
        )
        sigma_inelastic = (
            40 * sigma_inelastic_C
            + 80 * sigma_inelastic_H
            + sigma_inelastic_N
            + 8 * sigma_inelastic_O
            + sigma_inelastic_P
        )
    ...
    lamda_inelastic = 1.0 / (concentration * sigma_inelastic * 1e-18)
