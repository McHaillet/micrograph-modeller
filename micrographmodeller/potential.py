...
def wrapper(filepath, output_folder, voxel_size, skip_structure_edit=False, oversampling=1, binning=1,
    ...
    ep = ElectrostaticPotential(filepath, solvent_exclusion=solvent_exclusion,
                                solvent_potential=solvent_potential * solvent_factor,
                                absorption_contrast=absorption_contrast, voltage=voltage)
    v_atom = ep.sample_to_box(voxel_size=voxel_size, oversampling=oversampling, center_coordinates_in_box=True,
                              overhang=30, split=1, gpu_id=gpu_id, cores=cores)
    ...
    if binning > 1:
        ...
        downsampled = ndimage.zoom(support.reduce_resolution_real(v_atom, voxel_size, voxel_size * 2 * binning),
                                   1 / binning, order=3)
        ...
        if np.iscomplexobj(v_atom):
            ...
            print(f'writing real and imaginary part with name {output_name}')
            support.write_mrc(os.path.join(output_folder, f'{output_name}_real.mrc'), downsampled.real,
                              voxel_size*binning)
            support.write_mrc(os.path.join(output_folder, f'{output_name}_imag_{voltage * 1E-3:.0f}V.mrc'),
                              downsampled.imag, voxel_size*binning)
            ...
            print(f'writing real part with name {output_name}')
            support.write_mrc(os.path.join(output_folder, f'{output_name}_real.mrc'), downsampled,
                              voxel_size * binning)
    return
